...
<script>
    ...
    function handleDragLeave(event) {
        event.currentTarget.classList.remove('ring-2', 'ring-primary-500');
    }
    ...
    function handleDrop(event) {
        ...
        if (draggedTaskId && newStatus) {
            ...
            .then(data => {
                if (data.success) {
                    // Move the card to the new column with animation
                    draggedElement.style.transition = 'opacity 0.3s ease-in-out';
    ...
    // Add drag leave event to all columns
