<div>
    ...
    <x-dialog-header
        ...
        subtitle="Completa el formulario para crear una tarea"
        dialog-id="create-task-dialog" />

    {{-- Content --}}
    <form wire:submit="save"
          @submit.prevent="$wire.set('assignedUsers', selectedUsers); $wire.call('save')"
          x-data
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 px-6 py-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div x-data="{
                    open: false,
                    selectedUsers: @js($assignedUsers ?? []),
                    ...
                    getSelectedUsersData() {
                        return this.users.filter(user => this.selectedUsers.indexOf(user.id) > -1);
                    }
                }" class="relative">
                    ...
                    <button type="button"
                            @click="open = !open"
                            class="relative w-full cursor-default rounded-md bg-white py-2 pl-3 pr-10 text-left text-neutral-900 shadow-sm ring-1 ring-inset ring-neutral-300 focus:outline-none focus:ring-2 focus:ring-primary-600 dark:bg-neutral-800 dark:text-white dark:ring-neutral-600 sm:text-sm">
                        <span class="block truncate" x-text="selectedUsers.length > 0 ? selectedUsers.length + ' usuario(s) seleccionado(s)' : 'Selecciona usuarios'"></span>
                    ...
                    <div x-show="open"
                         ...
                         class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-neutral-800 dark:ring-white/10 sm:text-sm"
                         style="display: none;">
